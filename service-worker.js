/* EntreLinhas PWA 1.0 — v475 */
'use strict';

const PWA_VERSION = '1.0.14-v475';
...
self.addEventListener('fetch', function(event) {
  ...
  if (request.mode === 'navigate') {
    event.respondWith((async function() {
      ...
      if (cached) {
        event.waitUntil(refreshIndex().catch(function() { return undefined; }));
        return cached;
      }
      ...
      try {
        ...
        const fresh = await Promise.race([refreshIndex(), timeout]);
